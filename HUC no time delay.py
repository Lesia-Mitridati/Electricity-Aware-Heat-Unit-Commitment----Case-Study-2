...
#                
    ...
    def _build_objective(self,D_day): # building the objective function for the HUC
        ...
        m.setObjective(    - (sum(c_heat_bid['HUC',g,D_day,t] for g in HP+incinerator+boiler for t in time) + sum(c_heat_bid['HUC',g,D_day,t][0] for g in CHP for t in time) )/(len(time)*len(HP+CHP+CHP+incinerator+boiler))*gb.quicksum(self.variables.energy_stored[g,time[-1]] for g in heat_storage)   + gb.quicksum(500*self.variables.HS[n,t] for n in node for t in time) + gb.quicksum(c_heat_bid['HUC',g,D_day,t][x]*self.variables.Q[g,t,x] for t in time for g in CHP for x in range(2)) + gb.quicksum(c_heat_bid['HUC',g,D_day,t]*self.variables.Q[g,t] for t in time for g in incinerator+HP+boiler) + gb.quicksum(self.variables.r[h,t] + c_0[h]*self.variables.u[h,t] for t in time for h in CHP+incinerator+HP+boiler) ,   
            gb.GRB.MINIMIZE)
            #   - (sum(c_heat_bid['HUC',g,D_day,t] for g in HP+incinerator+boiler for t in time) + sum(c_heat_bid['HUC',g,D_day,t][0] for g in CHP for t in time) )/(len(time)*len(HP+CHP+CHP+incinerator+boiler))*gb.quicksum( self.variables.HF_in[n1,n2,t] - self.variables.HF_out[n1,n2,t] for n1 in node for n2 in node_node[n1] for t in time) 
...
#                    heat_load[n1,hiy[D_day,t]]-self.variables.HS[n1,t])        
        ...
        for t in time:
            for n1 in node:
                ...
                self.constraints.heat_balance[n1,t] = m.addConstr(
                    gb.quicksum(self.variables.Q_discharge[g,t] - self.variables.Q_charge[g,t] for g in heat_storage_node[n1]) + gb.quicksum(self.variables.Q[g,t] for g in prod_node[n1]) + gb.quicksum(self.variables.Q[g,t,x] for g in CHP_node[n1] for x in range(2)) + gb.quicksum( self.variables.HF_in[n2,n1,t] - self.variables.HF_in[n1,n2,t] for n2 in node_node[n1]),
                    gb.GRB.EQUAL,
                    heat_load[n1,hiy[D_day,t]]-self.variables.HS[n1,t])  
...
#                    heat_load[n1,hiy[D_day,t]]-self.variables.HS[n1,t])        
        ...
        for t in time:
            for n1 in node:
                ...
                self.constraints.heat_balance[n1,t] = m.addConstr(
                    gb.quicksum(self.variables.Q_discharge[g,t] - self.variables.Q_charge[g,t] for g in heat_storage_node[n1]) + gb.quicksum(self.variables.Q[g,t] for g in prod_node[n1]+CHP_node[n1]) + gb.quicksum(self.variables.HF_in[n2,n1,t] - self.variables.HF_in[n1,n2,t] for n2 in node_node[n1]),
                    gb.GRB.EQUAL,
                    heat_load[n1,hiy[D_day,t]]-self.variables.HS[n1,t])   
...
#        self.variables.mu_c3 = {} # HF in max / direction
#        self.variables.mu_c4 = {} # HF out max / direction
#
...
#                    self.variables.lambda_c7[n1,n2,t] = m.addVar(lb=0)
        ...
        self.variables.mu_c3 = {} # HF in max / direction
        self.variables.mu_c5a = {} # HF in lower
        ...
        for n1 in node:
            for n2 in node_node[n1]:
                for t in time:
                    ...
                    self.variables.mu_c3[n1,n2,t] = m.addVar(lb=0)
                    self.variables.mu_c5a[n1,n2,t] = m.addVar(lb=0)
        ...
        for t in time:
            for g in heat_storage:
                ...
                self.variables.lambda_c9[g,t] = m.addVar(lb=-gb.GRB.INFINITY)
        ...
        for t in time:
            for l in line:
                self.variables.mu_c22a[l,t] = m.addVar(lb=0)
                self.variables.mu_c22b[l,t] = m.addVar(lb=0)
        ...
        #McCormick relaxation variables!
        ...
        for t in time:
            for g in incinerator+HP+boiler:
                ...
                self.variables.MC1a[g,t] = m.addVar(lb=0)
                self.variables.MC1b[g,t] = m.addVar(lb=0)
            ...
            for g in CHP:
                ...
                for x in range(2):
                    ...
                    self.variables.MC1b[g,t,x] = m.addVar(lb=0)
        ...
        for t in time:
            for n1 in node:
                for n2 in node_node[n1]:
                    ...
                    self.variables.MC3[n1,n2,t] = m.addVar(lb=0)

        self.variables.MC14 = {}
        self.variables.MC16a = {}
        self.variables.MC16b = {}
        
        for t in time:
            for g in CHP:
                
                self.variables.MC14[g,t] = m.addVar(lb=0)
                self.variables.MC16a[g,t] = m.addVar(lb=0)
                self.variables.MC16b[g,t] = m.addVar(lb=0)
    ...
    def _build_objective(self,D_day): # building the objective function for the EAHUC
        ...
        m.setObjective( alpha*(   - (sum(c_heat_bid['EAHUC',g,D_day,t] for g in HP+incinerator+boiler for t in time) + sum(c_heat_bid['EAHUC',g,D_day,t][0] for g in CHP for t in time) )/(len(time)*len(HP+CHP+CHP+incinerator+boiler))*gb.quicksum(self.variables.energy_stored[g,time[-1]] for g in heat_storage) + gb.quicksum(500*self.variables.HS[n,t] for n in node for t in time) + gb.quicksum(c_heat_bid['EAHUC',g,D_day,t][x]*self.variables.Q[g,t,x] for t in time for g in CHP for x in range(2)) + gb.quicksum(c_heat_bid['EAHUC',g,D_day,t]*self.variables.Q[g,t] for t in time for g in incinerator+HP+boiler) + gb.quicksum(self.variables.r[h,t] + c_0[h]*self.variables.u[h,t] for t in time for h in CHP+incinerator+HP+boiler) ) +  (1-alpha)*( gb.quicksum(c_elec[g]*self.variables.P[g,t] for t in time for g in CHP+gen) + gb.quicksum(500*self.variables.ES[n,t] for t in time for n in zone_DK) ) ,   
            gb.GRB.MINIMIZE)
        ## - (sum(c_heat_bid['EAHUC',g,D_day,t] for g in HP+incinerator+boiler for t in time) + sum(c_heat_bid['EAHUC',g,D_day,t][0] for g in CHP for t in time) )/(len(time)*len(HP+CHP+CHP+incinerator+boiler))*gb.quicksum( self.variables.HF_in[n1,n2,t] - self.variables.HF_out[n1,n2,t] for n1 in node for n2 in node_node[n1] for t in time)  
        ...
        for t in time:
            for g in CHP:
                self.constraints.bid_validity_1[g,t,0] = m.addConstr(
                    ...
                    gb.GRB.GREATER_EQUAL,
                    rho_heat[g]/((1-alpha)*rho_elec[g])*self.variables.elec_price[zone_prod[g],t])
                ...
                self.constraints.bid_validity_2[g,t] = m.addConstr(
                    c_heat_bid['EAHUC',g,D_day,t][1] + 501*(1-self.variables.bid_accepted[g,t,1]),
                    gb.GRB.GREATER_EQUAL,
                    (c_elec[g] - self.variables.elec_price[zone_prod[g],t]/(1-alpha))*rmin[g] + c_heat[g])

            for g in HP:
                self.constraints.bid_validity_2[g,t] = m.addConstr(
                    c_heat_bid['EAHUC',g,D_day,t] + 501*(1-self.variables.u[g,t]),
                    gb.GRB.GREATER_EQUAL,
                    self.variables.elec_price[zone_prod[g],t]*rmin[g]/(1-alpha))
...
#                    heat_load[n1,hiy[D_day,t]]-self.variables.HS[n1,t])        
        ...
        for t in time:
            for g in CHP:
                ...
                self.constraints.pmax_bid_lower[g,t] = m.addConstr(
                ...
                self.constraints.pmax_bid_upper[g,t] = m.addConstr(
                    - self.variables.pmax_bid[g,t] + (fmax[g]-rho_heat[g]*gb.quicksum(self.variables.Q[g,t,x] for x in range(2)))/rho_elec[g],
                    gb.GRB.LESS_EQUAL,
        ...
        for t in time:
            for g in boiler:
                ...
                self.constraints.dL_dQ[g,t] = m.addConstr(
                    alpha*c_heat_bid['EAHUC',g,D_day,t]-self.variables.mu_c1a[g,t]+self.variables.mu_c1b[g,t]-self.variables.heat_price[node_prod[g],t],
                    gb.GRB.EQUAL,
                    0)
            ...
            for g in HP:
                ...
                self.constraints.dL_dQ[g,t] = m.addConstr(
                    alpha*c_heat_bid['EAHUC',g,D_day,t]-self.variables.mu_c1a[g,t]+self.variables.mu_c1b[g,t]-self.variables.heat_price[node_prod[g],t]+self.variables.elec_price[zone_prod[g],t],
                    gb.GRB.EQUAL,
                    0)
            ...
            for g in CHP:
                ...
                self.constraints.dL_dQ[g,t,0] = m.addConstr(
                    alpha*c_heat_bid['EAHUC',g,D_day,t][0]-self.variables.mu_c1a[g,t]+self.variables.mu_c1b[g,t,0]-self.variables.heat_price[node_prod[g],t] - rho_heat[g]/rho_elec[g]*self.variables.mu_c14[g,t] + rmin[g]*self.variables.mu_c15[g,t] + rho_heat[g]/rho_elec[g]*self.variables.mu_c16a[g,t] - rho_heat[g]/rho_elec[g]*self.variables.mu_c16b[g,t],
                    gb.GRB.EQUAL,
                    0)        

                self.constraints.dL_dQ[g,t,1] = m.addConstr(
                    alpha*c_heat_bid['EAHUC',g,D_day,t][1]+self.variables.mu_c1b[g,t,1]-self.variables.heat_price[node_prod[g],t] - rho_heat[g]/rho_elec[g]*self.variables.mu_c14[g,t] + rmin[g]*self.variables.mu_c15[g,t] + rho_heat[g]/rho_elec[g]*self.variables.mu_c16a[g,t] - rho_heat[g]/rho_elec[g]*self.variables.mu_c16b[g,t],
                    gb.GRB.EQUAL,
                    0) 
...
#                        0)
                    ...
                    self.constraints.dL_dHF_in[n1,n2,time[x]] = m.addConstr(
                        self.variables.mu_c3[n1,n2,time[x]] - self.variables.mu_c5a[n1,n2,time[x]] + self.variables.heat_price[n1,time[x]] - self.variables.heat_price[n2,time[x]] ,
                        gb.GRB.EQUAL,
                        0)
...
#                        0)
        ...
        for g in heat_storage:
            ...
            for (t,t2) in zip(time[:-1],time[1:]):
                ...
                self.constraints.dL_denergy_stored[g,t] = m.addConstr(
                    - self.variables.mu_c10a[g,t] + self.variables.mu_c10b[g,t] + self.variables.lambda_c9[g,t] - self.variables.lambda_c9[g,t2],
                    gb.GRB.EQUAL,
                    0)  

            self.constraints.dL_denergy_stored[g,time[-1]] = m.addConstr(
                - alpha*(sum(c_heat_bid['EAHUC',g,D_day,x] for g in HP+incinerator+boiler for x in time) + sum(c_heat_bid['EAHUC',g,D_day,x][0] for g in CHP for x in time) )/(len(time)*len(HP+CHP+CHP+incinerator+boiler)) - self.variables.mu_c10a[g,time[-1]] + self.variables.mu_c10b[g,time[-1]] + self.variables.lambda_c9[g,time[-1]],
                gb.GRB.EQUAL,
                0)
        ...
        self.constraints.strong_duality = m.addConstr( gb.quicksum(qmin[g]*self.variables.MC1a[g,t] - qmax[g]*self.variables.MC1b[g,t] for g in HP+incinerator+boiler for t in time) + gb.quicksum(q_bid[g][0]*self.variables.MC1a[g,t] - gb.quicksum(q_bid[g][x]*self.variables.MC1b[g,t,x] for x in range(2)) for g in CHP for t in time) + gb.quicksum( - heat_load[n1,hiy[D_day,t]]*self.variables.mu_c2b[n1,t] for n1 in node for t in time) + gb.quicksum( -f_upper[n1,n2]*self.variables.MC3[n1,n2,t] for n1 in node for n2 in node_node[n1] for t in time) + gb.quicksum( heat_load[n1,hiy[D_day,t]]*self.variables.heat_price[n1,t] for n1 in node for t in time) + gb.quicksum( - emax[g]*self.variables.mu_c10b[g,t] - qmax[g]*self.variables.mu_c11b[g,t] - qmax[g]*self.variables.mu_c12b[g,t] for g in heat_storage for t in time) + gb.quicksum( fmin[g]/rho_elec[g]*self.variables.MC14[g,t] + fmax[g]/rho_elec[g]*(-self.variables.MC16a[g,t]+self.variables.MC16b[g,t]) for g in CHP for t in time) + gb.quicksum( pmin[g]*u_set['EAHUC',g,D_day,t]*self.variables.mu_c18a[g,t] - pmax[g]*u_set['EAHUC',g,D_day,t]*self.variables.mu_c18a[g,t] for g in gen for t in time) + gb.quicksum( - elec_load[z,hiy[D_day,t]]*self.variables.mu_c19b[z,t] - solar_prod[z,hiy[D_day,t]]*self.variables.mu_c20b[z,t] - wind_prod[z,hiy[D_day,t]]*self.variables.mu_c21b[z,t] for z in zone_DK for t in time)  + gb.quicksum( - ATC_lower[l,hiy[D_day,t]]*self.variables.mu_c22a[l,t] - ATC_upper[l,hiy[D_day,t]]*self.variables.mu_c22b[l,t] for l in line for t in time) + gb.quicksum(elec_load[z,hiy[D_day,t]]*self.variables.elec_price[z,t] for z in zone_DK for t in time),
                gb.GRB.EQUAL,
                alpha*(- (sum(c_heat_bid['EAHUC',g,D_day,t] for g in HP+incinerator+boiler for t in time) + sum(c_heat_bid['EAHUC',g,D_day,t][0] for g in CHP for t in time) )/(len(time)*len(HP+CHP+CHP+incinerator+boiler))*gb.quicksum(self.variables.energy_stored[g,time[-1]] for g in heat_storage) + gb.quicksum(500*self.variables.HS[n,t] for n in node for t in time) + gb.quicksum(c_heat_bid['EAHUC',g,D_day,t][x]*self.variables.Q[g,t,x] for t in time for g in CHP for x in range(2)) + gb.quicksum(c_heat_bid['EAHUC',g,D_day,t]*self.variables.Q[g,t] for t in time for g in incinerator+HP+boiler) ) +  (1-alpha)*( gb.quicksum(c_elec[g]*self.variables.P[g,t] for t in time for g in CHP+gen) + gb.quicksum(500*self.variables.ES[n,t] for t in time for n in zone_DK) ) )

        # McCormick envelopes (EXACT) of bilinear term u*\mu in strong duality!                     

        self.constraints.MC1a_upper_1 = {} 
        self.constraints.MC1a_lower_1 = {} 
        self.constraints.MC1a_upper_2 = {} 
        self.constraints.MC1a_lower_2 = {} 
        
        for t in time:
            for g in incinerator+HP+boiler:
                
                self.constraints.MC1a_lower_1[g,t] = m.addConstr(
                    self.variables.MC1a[g,t],
                    gb.GRB.GREATER_EQUAL,
                    0)

                self.constraints.MC1a_upper_1[g,t] = m.addConstr(
                    self.variables.MC1a[g,t],
                    gb.GRB.LESS_EQUAL,
                    M*self.variables.u[g,t])

                self.constraints.MC1a_lower_2[g,t] = m.addConstr(
                    self.variables.MC1a[g,t] - self.variables.mu_c1a[g,t],
                    gb.GRB.GREATER_EQUAL,
                    -M*(1-self.variables.u[g,t]))

                self.constraints.MC1a_upper_2[g,t] = m.addConstr(
                    self.variables.MC1a[g,t] - self.variables.mu_c1a[g,t],
                    gb.GRB.LESS_EQUAL,
                    0)
            ...
            for g in CHP:
                ...
                self.constraints.MC1a_upper_1[g,t] = m.addConstr(
                    self.variables.MC1a[g,t],
                    gb.GRB.LESS_EQUAL,
                    M*self.variables.bid_accepted[g,t,1])

                self.constraints.MC1a_lower_2[g,t] = m.addConstr(
                    self.variables.MC1a[g,t] - self.variables.mu_c1a[g,t],
                    gb.GRB.GREATER_EQUAL,
                    -M*(1-self.variables.bid_accepted[g,t,1]))
        ...
        self.constraints.MC1b_upper_1 = {} 
        self.constraints.MC1b_lower_1 = {} 
        self.constraints.MC1b_upper_2 = {} 
        self.constraints.MC1b_lower_2 = {} 
        
        for t in time:
            for g in incinerator+HP+boiler:
                
                self.constraints.MC1b_lower_1[g,t] = m.addConstr(
                    self.variables.MC1b[g,t],
                    gb.GRB.GREATER_EQUAL,
                    0)

                self.constraints.MC1b_upper_1[g,t] = m.addConstr(
                    self.variables.MC1b[g,t],
                    gb.GRB.LESS_EQUAL,
                    M*self.variables.u[g,t])

                self.constraints.MC1b_lower_2[g,t] = m.addConstr(
                    self.variables.MC1b[g,t] - self.variables.mu_c1b[g,t],
                    gb.GRB.GREATER_EQUAL,
                    -M*(1-self.variables.u[g,t]))

                self.constraints.MC1b_upper_2[g,t] = m.addConstr(
                    self.variables.MC1b[g,t] - self.variables.mu_c1b[g,t],
                    gb.GRB.LESS_EQUAL,
                    0)
            ...
            for g in CHP:
                for x in range(2):
                    ...
                    self.constraints.MC1b_lower_1[g,t,x] = m.addConstr(
                        self.variables.MC1b[g,t,x],
                        gb.GRB.GREATER_EQUAL,
                        0)
    
                    self.constraints.MC1b_upper_1[g,t,x] = m.addConstr(
                        self.variables.MC1b[g,t,x],
                        gb.GRB.LESS_EQUAL,
                        M*self.variables.bid_accepted[g,t,x])
    
                    self.constraints.MC1b_lower_2[g,t,x] = m.addConstr(
                        self.variables.MC1b[g,t,x] - self.variables.mu_c1b[g,t,x],
                        gb.GRB.GREATER_EQUAL,
                        -M*(1-self.variables.bid_accepted[g,t,x]))
    
                    self.constraints.MC1b_upper_2[g,t,x] = m.addConstr(
                        self.variables.MC1b[g,t,x] - self.variables.mu_c1b[g,t,x],
                        gb.GRB.LESS_EQUAL,
                        0)
        ...
        self.constraints.MC3_upper_1 = {} 
        self.constraints.MC3_lower_1 = {} 
        self.constraints.MC3_upper_2 = {} 
        self.constraints.MC3_lower_2 = {} 
        
        for t in time:
            for n1 in node:
                for n2 in node_node[n1]:
                
                    self.constraints.MC3_lower_1[n1,n2,t] = m.addConstr(
                        self.variables.MC3[n1,n2,t],
                        gb.GRB.GREATER_EQUAL,
                        0)
    
                    self.constraints.MC3_upper_1[n1,n2,t] = m.addConstr(
                        self.variables.MC3[n1,n2,t],
                        gb.GRB.LESS_EQUAL,
                        M*self.variables.HF_direction[n1,n2,t])
    
                    self.constraints.MC3_lower_2[g,t] = m.addConstr(
                        self.variables.MC3[n1,n2,t] - self.variables.mu_c3[n1,n2,t],
                        gb.GRB.GREATER_EQUAL,
                        -M*(1-self.variables.HF_direction[n1,n2,t]))
    
                    self.constraints.MC3_upper_2[n1,n2,t] = m.addConstr(
                        self.variables.MC3[n1,n2,t] - self.variables.mu_c3[n1,n2,t],
                        gb.GRB.LESS_EQUAL,
                        0)
        ...
        self.constraints.MC14_upper_1 = {} 
        self.constraints.MC14_lower_1 = {} 
        self.constraints.MC14_upper_2 = {} 
        self.constraints.MC14_lower_2 = {} 
        
        for t in time:
            for g in CHP:
                
                self.constraints.MC14_lower_1[g,t] = m.addConstr(
                    self.variables.MC14[g,t],
                    gb.GRB.GREATER_EQUAL,
                    0)

                self.constraints.MC14_upper_1[g,t] = m.addConstr(
                    self.variables.MC14[g,t],
                    gb.GRB.LESS_EQUAL,
                    M*self.variables.u[g,t])

                self.constraints.MC14_lower_2[g,t] = m.addConstr(
                    self.variables.MC14[g,t] - self.variables.mu_c14[g,t],
                    gb.GRB.GREATER_EQUAL,
                    -M*(1-self.variables.u[g,t]))

                self.constraints.MC14_upper_2[g,t] = m.addConstr(
                    self.variables.MC14[g,t] - self.variables.mu_c14[g,t],
                    gb.GRB.LESS_EQUAL,
                    0)

        self.constraints.MC16a_upper_1 = {} 
        self.constraints.MC16a_lower_1 = {} 
        self.constraints.MC16a_upper_2 = {} 
        self.constraints.MC16a_lower_2 = {} 
        
        for t in time:
            for g in CHP:
                
                self.constraints.MC16a_lower_1[g,t] = m.addConstr(
                    self.variables.MC16a[g,t],
                    gb.GRB.GREATER_EQUAL,
                    0)

                self.constraints.MC16a_upper_1[g,t] = m.addConstr(
                    self.variables.MC16a[g,t],
                    gb.GRB.LESS_EQUAL,
                    M*self.variables.u[g,t])

                self.constraints.MC16a_lower_2[g,t] = m.addConstr(
                    self.variables.MC16a[g,t] - self.variables.mu_c16a[g,t],
                    gb.GRB.GREATER_EQUAL,
                    -M*(1-self.variables.u[g,t]))

                self.constraints.MC16a_upper_2[g,t] = m.addConstr(
                    self.variables.MC16a[g,t] - self.variables.mu_c16a[g,t],
                    gb.GRB.LESS_EQUAL,
                    0)
                
        self.constraints.MC16b_upper_1 = {} 
        self.constraints.MC16b_lower_1 = {} 
        self.constraints.MC16b_upper_2 = {} 
        self.constraints.MC16b_lower_2 = {} 
        
        for t in time:
            for g in CHP:
                
                self.constraints.MC16b_lower_1[g,t] = m.addConstr(
                    self.variables.MC16b[g,t],
                    gb.GRB.GREATER_EQUAL,
                    0)

                self.constraints.MC16b_upper_1[g,t] = m.addConstr(
                    self.variables.MC16b[g,t],
                    gb.GRB.LESS_EQUAL,
                    M*self.variables.u[g,t])

                self.constraints.MC16b_lower_2[g,t] = m.addConstr(
                    self.variables.MC16b[g,t] - self.variables.mu_c16b[g,t],
                    gb.GRB.GREATER_EQUAL,
                    -M*(1-self.variables.u[g,t]))

                self.constraints.MC16b_upper_2[g,t] = m.addConstr(
                    self.variables.MC16b[g,t] - self.variables.mu_c16b[g,t],
                    gb.GRB.LESS_EQUAL,
                    0)                
